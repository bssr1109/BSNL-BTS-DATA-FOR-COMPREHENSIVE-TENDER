...
LABELS = {
    "si_no":"SI NO", "ba_name":"BA Name", "oa_name":"OA Name", "sdca_name":"SDCA Name",
    "bts_type":"BTS Type", "tower_type":"Tower Type", "bts_name":"BTS Name", "rp_id":"RP ID",
    "site_type":"Site Type", "sites_having_acs":"Sites Having ACS", "acs_count":"ACS Count",
    "make":"Make", "ac_capacity":"AC Capacity", "status_of_ac":"Status of AC", "ac_pm_cm":"AC PM/CM",
    "free_cooling_available":"Free Cooling Available", "site_load_gt_10kw":"Site Load >10KW",
    "remarks":"Remarks", "sde_name":"Name of SDE CM/CFA", "is_completed":"Completed",
    "last_updated_by":"Last Updated By", "last_updated_at":"Last Updated At"
}
...
if mode == "Field User":
    ...
    st.markdown(f"<div class='small-muted'>SDCA: <b>{sdca}</b> | SDE: <b>{sde_name}</b></div>", unsafe_allow_html=True)
    ...
    if search.strip():
        m = df["bts_name"].astype(str).str.contains(search, case=False, na=False) | df["rp_id"].astype(str).str.contains(search, case=False, na=False)
        choices = df[m].index.tolist()
        if choices:
            labels = [f"{int(df.loc[i,'si_no']) if not is_blank(df.loc[i,'si_no']) else i+1}. {df.loc[i,'bts_name']} - {df.loc[i,'rp_id']}" for i in choices]
    ...
    st.markdown(f"<div class='site-card'><div class='big-title'>Site {pos+1} of {total}</div>"
                f"<b>BTS:</b> {row.get('bts_name','')}<br>"
                f"<b>RP ID:</b> {row.get('rp_id','')}<br>"
                f"<span class='small-muted'>BA: {row.get('ba_name','')} | OA: {row.get('oa_name','')} | BTS Type: {row.get('bts_type','')}</span></div>",
                unsafe_allow_html=True)

    with st.form(key=f"site_form_{row['id']}", clear_on_submit=False):
        tower_type = st.selectbox("Tower Type *", [""] + options["tower_type"], index=selected_index(options["tower_type"], row.get("tower_type")))
        site_type = st.selectbox("Site Type *", [""] + options["site_type"], index=selected_index(options["site_type"], row.get("site_type")))
        sites_having_acs = st.selectbox("Sites Having ACS *", [""] + options["sites_having_acs"], index=selected_index(options["sites_having_acs"], row.get("sites_having_acs")))
        ...
        if sites_having_acs == "No":
            ...
            acs_count = st.number_input("ACS Count *", min_value=0, step=1, value=int(row.get("acs_count") or 0))
            make = st.text_input("Make *", value="" if is_blank(row.get("make")) else str(row.get("make")))
            ac_capacity = st.text_input("AC Capacity *", value="" if is_blank(row.get("ac_capacity")) else str(row.get("ac_capacity")))
            status_of_ac = st.selectbox("Status of AC *", [""] + options["status_of_ac"], index=selected_index(options["status_of_ac"], row.get("status_of_ac")))
            ac_pm_cm = st.selectbox("AC PM/CM *", [""] + options["ac_pm_cm"], index=selected_index(options["ac_pm_cm"], row.get("ac_pm_cm")))

        free_cooling_available = st.selectbox("Free Cooling Available *", [""] + options["free_cooling_available"], index=selected_index(options["free_cooling_available"], row.get("free_cooling_available")))
        site_load_gt_10kw = st.selectbox("Site Load >10KW *", [""] + options["site_load_gt_10kw"], index=selected_index(options["site_load_gt_10kw"], row.get("site_load_gt_10kw")))
        remarks = st.text_area("Remarks", value="" if is_blank(row.get("remarks")) else str(row.get("remarks")), height=90)
    ...
    if not current_complete:
        st.markdown("<div class='status-warn'>Fill all fields marked * to enable Save & Next.</div>", unsafe_allow_html=True)
    else:
        st.markdown("<div class='status-ok'>This site is ready to save.</div>", unsafe_allow_html=True)
    ...
    if search.strip():
        ...
        for col in ["bts_name", "rp_id", "sde_name", "sdca_name"]:
            m = m | view[col].astype(str).str.contains(search, case=False, na=False) if isinstance(m, pd.Series) else view[col].astype(str).str.contains(search, case=False, na=False)
        view = view[m]

    cols = [c for c in DISPLAY_COLUMNS if c in view.columns] + [c for c in ["last_updated_by", "last_updated_at"] if c in view.columns]
